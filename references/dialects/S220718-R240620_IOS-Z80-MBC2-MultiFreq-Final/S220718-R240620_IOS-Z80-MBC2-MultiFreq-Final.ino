...
EDITED BY SKL TO ALLOW MULTIPLE Z80 TYPES AND FREQUENCIES
...
S220718-R240620   Added support for Collapse Os (https://collapseos.org/).
     -MultiFreq   EDITED BY SKL TO ALLOW MULTIPLE Z80 TYPES AND FREQUENCIES
...
#else
  #define CLOCK_16    "1.6"               // added by SKL
  #define CLOCK_20    "2.0"               // added by SKL
  #define CLOCK_27    "2.7"               // added by SKL
  #define CLOCK_40    "4.0"               // added by SKL
  #define CLOCK_80    "8.0"               // added by SKL
  // #define CLOCK_HIGH  "8"              // commented out by SKL
  // #define CLOCK_LOW   "4"              // commented out by SKL
#endif
...
#include <pff.h>                          // Light handler for FAT16 and FAT32 filesystems on SD
...
const byte    clockModeAddr = 13;         // Internal EEPROM address                // deleted by SKL: for the Z80 clock high/low speed switch
                                          // Holds clockMode +                      // deleted by SKL: (1 = low speed, 0 = high speed)
                                          //             0 if OG Z80 (max 2.6MHz)     - added by SKL
                                          //            10 if Z80A (max 4MHz)         - added by SKL
                                          //            20 if 'modern' Z80 (max 8MHz) - added by SKL
...
char          inChar2;                    // Input char from serial - added by SKL
char          inChar3;                    // Input char from serial - added by SKL
byte          iCount;                     // Temporary variable (counter)
// byte          clockMode;               // Z80 clock HI/LO speed selector (0 = 8/10MHz, 1 = 4/5MHz) - commmented out by SKL
byte          clockMode;                  // Z80 clock selector (0 = 8MHz, 1 = 4MHz, 2 = 2.7MHz, 3 = 2MHz, 4 = 1.6MHz) - added by SKL
                                          //                (20MHz crystal will give proportional values) - added by SKL
byte          z80Type;                    // type of Z80 : 0 if OG Z80 (max 2.6MHz), 1 if Z80A (max 4MHz), 2 if 'modern' Z80 (max 8MHz) - added by SKL
...
// ------------------------------------------------------------------------------
  ...
  char          minZ80TypeChar = '1';       // Minimum allowed ASCII value selection (z80 type selection) - added by SKL
  char          maxZ80TypeChar = '3';       // Maximum allowed ASCII value selection (z80 type selection) - added by SKL
  char          minClockSpeedChar = '1';       // Minimum allowed ASCII value selection (clock speed selection) - added by SKL
  char          maxClockSpeedChar = '3';       // Maximum allowed ASCII value selection (clock speed selection) - added by SKL
                                               // Changes depending on Z80 Type
  ...
  byte          minClockMode  = 4;          // Minimum value of clockMode - depends on Z80 type - 4 is lowest speed - added by SKL
  byte          maxClockMode  = 4;          // Maximum value of clockMode - 4 for all Z80 types - added by SKL
  bool          clockModeZ80Error = false;  // Used when checking EEPROM stored values for clockMode and z80Type
...
// ------------------------------------------------------------------------------
  ...
  // Read the Z80 CPU type and speed mode         // find out what sort of Z80 we're using and set maxClockMode and fix if necessary - added by SKL
  clockMode = EEPROM.read(clockModeAddr)%10;      // read what's stored in EEPROM for clockMode itself
  z80Type = EEPROM.read(clockModeAddr)/10;        // read what's stored in EEPROM for z80Type
  switch(z80Type)
  {
    case 0:                                       //  OG Z80
      minClockMode = 2;                           //  max speed 2.7MHz
      break;
    case 1:                                       //  Z80A
      minClockMode = 1;                           //  max speed 4MHz
      break;
    case 2:                                       // 'modern' Z80
      minClockMode = 0;                           //  max speed 8MHz
      break;
    default:
      clockModeZ80Error = true;                  // Z80Type stored incorrectly: set error flag (minClockMode will be 4)
      break;
  }
  if (clockMode > maxClockMode || clockMode < minClockMode)   // error if clockMode is out-of-range (>4) or too low for current Z80Type: set error flag
  {
    clockModeZ80Error = true;
  }
  if (clockModeZ80Error)
  {
    EEPROM.update(clockModeAddr, 4);              // set EEPROM to hold 04: OG Z80 at 1.6MHz for safety
    clockMode = EEPROM.read(clockModeAddr)%10;    // reread what's stored in EEPROM for clockMode itself
    z80Type = EEPROM.read(clockModeAddr)/10;      // reread what's stored in EEPROM for z80Type
    minClockMode = 2;                             // set the minClockMode, which will be 2
  }
  
  
//  if (EEPROM.read(clockModeAddr) > 1)  // Check clockMode is a valid value, otherwise set it to low speed - deleted by SKL
//  {
//    EEPROM.update(clockModeAddr, 1);              // deleted by SKL
//  }
//  clockMode = EEPROM.read(clockModeAddr);         // Read the previous stored value - deleted by SKL
  ...
  Serial.begin(115200);
  // Serial.println(F("\r\n\nZ80-MBC2 - A040618\r\nI/O Subsystem (IOS) - S220718-R240620-MultiFreq\r\n"));
  Serial.println();
  // Serial.println(F("  +----------------------------------------------------+"));
  // Serial.println(F("  |                 Z80-MBC2 - A040618                 |"));
  // Serial.println(F("  |  I/O Subsystem (IOS) - S220718-R240620  MultiFreq  |"));
  // Serial.println(F("  +----------------------------------------------------+"));  
  Serial.println();
  Serial.println(F("--  .---..-.  .-.      .    ..--.  .--. .-.   ---------------------------------"));
  Serial.println(F("       /(   ):   :     |\\  /||   ):    (   )       board rev. A040618"));
  Serial.println(F("      /  >-< |   | ___ | \\/ ||--: |      .'        IOS rev.   S220718-R240620"));
  Serial.println(F("     /  (   ):   ;     |    ||   ):     /                     +multifrequency"));
  Serial.println(F("--  '---'`-'  `-'      '    ''--'  `--''---'  ---------------------------------"));
  Serial.println();
  ...
  // Print the Z80 Type and maximum speed - added by SKL
  Serial.println(F("IOS    Multifrequency support enabled"));
  switch(z80Type)
  {
    case 0:
      Serial.print(F("           'OG' Z80 capable of "));
      break;
    case 1:   
      Serial.print(F("           'Classic' Z80A capable of "));
      break;
    case 2:
      Serial.print(F("           'Modern' Z80 capable of "));
      break;
    default:
      Serial.print(F("ERROR: Z80 TYPE NOT RECOGNIZED capable of "));
      break;
  }
  switch(minClockMode)
  {
    case 0:
      Serial.print(CLOCK_80);
      break;
    case 1:   
      Serial.print(CLOCK_40);
      break;
    case 2:
      Serial.print(CLOCK_27);
      break;
    default:
      Serial.print(F("ERROR: MAXIMUM CLOCK NOT RECOGNIZED"));
      break;  
  }
  ...
  Serial.print(F("           Z80 clock currently set at "));
  switch(clockMode)
  {
    case 0:
      Serial.print(CLOCK_80);
    ...
    case 2:
      ...
      break;
    case 3:
      Serial.print(CLOCK_20);
      break;
    case 4:
      Serial.print(CLOCK_16);
      break;
    default:
      Serial.print(F("ERROR: CLOCK NOT RECOGNIZED"));
      break;  
  }
//  if (clockMode) Serial.print(CLOCK_LOW); - commented out by SKL
//  else Serial.print(CLOCK_HIGH); - commented out by SKL
  Serial.println(F(" MHz"));
...
//    Serial.print(F(" 6: Change Z80 clock speed (->")); - commented out by SKL
//    if (clockMode) Serial.print(CLOCK_HIGH); - commented out by SKL
//    else Serial.print(CLOCK_LOW); - commented out by SKL
//    Serial.println(F("MHz)")); - commented out by SKL
    Serial.println(F("    6  [multifreq] Change Z80 type and clock speed")); // - added by SKL
    ...
    switch (inChar)
    {
      case '6':                                   // Change the Z80 Type and clock speed of the Z80 CPU - amended by SKL
//      clockMode = !clockMode;                   // Toggle Z80 clock speed mode (High/Low) - commented out by SKL
//      EEPROM.update(clockModeAddr, clockMode);  // Save it to the internal EEPROM - commented out by SKL
                                                  // rest of case added by SKL
        while (Serial.available() > 0)                // Flush input serial Rx buffer
        {
         Serial.read();
        }
        Serial.println();                         // select Z80 Type
        Serial.println(F("Select Z80 type."));
        Serial.println(F("    1  'OG' Z80 at maximum 2.7 MHz"));
        Serial.println(F("    2  'Classic' Z80A at maximum 4.0 MHz"));
        Serial.println(F("    3  'Modern' Z80 at maximum 8.0 MHz"));
        timeStamp = millis();
        Serial.print(F("Enter your choice > "));
        do
        {
        ...
        while ((inChar2 < minZ80TypeChar) || (inChar2 > maxZ80TypeChar));
        ...
        switch (inChar2)
        {
          case '1':
            z80Type = 0;
            minClockMode = 2;
            maxClockSpeedChar = '3';              // can select maximum of 2.7MHz
            break;
          case '2':
            z80Type = 1;
            minClockMode = 1;
            maxClockSpeedChar = '4';              // can select maximum of 4MHz
            break;
          case '3':
            z80Type = 2;
            minClockMode = 0;
            maxClockSpeedChar = '5';              // can select maximum of 8MHz
            break;
        }
        ...
        Serial.println(F("Select Z80 clock speed."));
        Serial.println(F("    1  1.6 MHz"));
        Serial.println(F("    2  2.0 MHz"));
        Serial.println(F("    3  2.7 MHz"));
        if (minClockMode <= 1)
        {
          Serial.println(F("    4  4.0 MHz"));
          if (minClockMode == 0)
          {
            Serial.println(F("    5  8.0 MHz"));
          }
        }
        timeStamp = millis();
        Serial.print(F("Enter your choice > "));
        ...
        while ((inChar3 < minClockSpeedChar) || (inChar3 > maxClockSpeedChar));
      ...
      EEPROM.update(clockModeAddr, ( (z80Type * 10) + clockMode) );     // we store the new values in the EEPROM as zc with z=z80Type and c = clockMode
      Serial.println();                         // select Z80 Type
      Serial.println(F("IOS[multifreq]    Press RESET to restart at new frequency."));
      while(true){}
      break;
  ...
  if (bootMode == 2)
  {
    Serial.print(F("IOS    Using disks: "));
    printOsName(diskSet);
    Serial.println();
  }
  ...
  TCCR2 &= ~(1 << COM21);
  OCR2 = clockMode;                               // Set the compare value to toggle OC2  - now 0, 1, 2, 3 or 4 - amended by SKL
  pinMode(CLK, OUTPUT);                           // Set OC2 as output and start to output the clock
  Serial.println(F("IOS    Z80 now running"));
  Serial.println();
...
// Return value: 0 if RTC not present, 1 if found.
{
  ...
  Serial.print(F("IOS    Found RTC DS3231 ("));
  printDateTime(1);
  Serial.print(F(" "));
...
//        call readSD() consecutively until EOF is reached
{
  UINT  numBytes;
  byte  errcode;
  errcode = PF.readFile(buffSD, 32, &numBytes);
...
// NOTE3: To finalize the current write operation a writeSD(NULL, &numWrittenBytes) must be called as last action
{
  ...
  if (buffSD != NULL)
  {
    errcode = PF.writeFile(buffSD, 32, &numBytes);
  }
  else
  {
    errcode = PF.writeFile(0, 0, &numBytes);
  }
...
{
  byte i;
  return PF.seek(((unsigned long) sectNum) << 9);
}
