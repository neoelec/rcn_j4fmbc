...
/* Test if CS is low */
static inline UINT IS_CS_LOW(void)
{
...
/* Arduino - SPI: Delay 100 microseconds */
static inline void dly_100us(void)
{
	delayMicroseconds(100);
}

/* Arduino - SPI: Initialize MMC control ports */
static inline void init_spi(void)
{
	SPI.begin();
}

/* Arduino - SPI: Send a byte to the MMC */
static inline void xmit_spi(BYTE d)
{
	SPI.transfer(d);
}

/* Arduino - SPI: Send a 0xFF to the MMC and get the received byte */
...
)
{
	...
	xmit_spi(static_cast<BYTE>(arg >> 24));		/* Argument[31..24] */
	xmit_spi(static_cast<BYTE>(arg >> 16));		/* Argument[23..16] */
	xmit_spi(static_cast<BYTE>(arg >> 8));			/* Argument[15..8] */
	xmit_spi(static_cast<BYTE>(arg));				/* Argument[7..0] */
