...
bool Z80Mbc2Cfg::begin(DevSd *sd, const char *cfg_name)
{
  bool is_ok;
...
bool Z80Mbc2Cfg::__begin_w_boot_mode(uint8_t boot_mode)
{
...
bool Z80Mbc2Cfg::__begin_w_cfg_name(const char *cfg_name)
{
  char csv[SZ_CSV_BUF];
...
bool Z80Mbc2Cfg::__openCsv(const char *cfg_name)
{
  uint8_t operation;
  uint8_t error;
...
bool Z80Mbc2Cfg::__readCsv(char *csv, const char *cfg_name)
{
  char buf[SZ_BUF];
  uint8_t error;
  uint8_t operation;
...
void Z80Mbc2Cfg::__intNrBootMode(void)
{
  char cfg_name[] = CFG_NAME_FMT;
